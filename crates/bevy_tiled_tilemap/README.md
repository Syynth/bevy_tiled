...
This is a **Layer 3 plugin** that observes spawning events from `bevy_tiled_core` and adds rendering components using `bevy_ecs_tilemap` for optimal batched tile rendering.
...
- **Layer 1** (`bevy_tiled_assets`) - Loads and parses Tiled files
- **Layer 2** (`bevy_tiled_core`) - Spawns entities and emits events
- **Layer 3** (`bevy_tiled_tilemap`) - Observes events and adds rendering ← **This crate**
